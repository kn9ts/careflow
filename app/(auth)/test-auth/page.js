...
export default function TestAuthPage() {
  ...
  const runTests = async () => {
    ...
    if (currentUser) {
      ...
      const hasRequiredFields = requiredFields.every((field) => currentUser[field] !== undefined);
      addTestResult(
    ...
    try {
      ...
      addTestResult(
        ...
        response.ok,
        response.ok ? 'Successfully accessed protected endpoint' : `API returned ${response.status}`
      );
    } catch (err) {
    ...
    try {
      ...
      addTestResult(
        'Session Persistence',
        hasPersistentAuth,
        hasPersistentAuth ? 'Authentication state persists' : 'Authentication state not persistent'
      );
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="bg-background-card rounded-xl border border-white/10 p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">Authentication Test Suite</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-background-card rounded-xl border border-white/10 p-6">
            ...
            <p className="text-gray-400 text-sm">{token ? 'Token available' : 'No token'}</p>
          </div>
          <div className="bg-background-card rounded-xl border border-white/10 p-6">
            <h3 className="text-lg font-semibold text-white mb-2">Error</h3>
            <p className="text-gray-400 text-sm">{error || 'No errors'}</p>
          </div>
        </div>
        ...
        <div className="bg-background-card rounded-xl border border-white/10 p-6">
          ...
          <div className="text-gray-400 space-y-2 text-sm">
            <p>
              1. <strong>Configuration Test:</strong> Verifies Firebase configuration is properly
              set up
            </p>
            <p>
              2. <strong>Authentication State:</strong> Checks if user is authenticated and
              error-free
            </p>
            <p>
              3. <strong>User Object Structure:</strong> Validates user object has required fields
            </p>
            <p>
              4. <strong>Token Validation:</strong> Checks if authentication token is properly
              formatted
            </p>
            <p>
              5. <strong>Protected API Access:</strong> Tests access to protected endpoints
            </p>
            <p>
              6. <strong>Session Persistence:</strong> Verifies authentication state persists
            </p>
            <p className="mt-4 text-yellow-400">
              Note: This is a testing page. Use real credentials only in development environment.
            </p>
          </div>
        </div>
      </div>
