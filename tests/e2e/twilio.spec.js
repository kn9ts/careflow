...
test.describe('Twilio Call Initiation', () => {
  ...
  test('should initiate PSTN call to phone number', async ({ page }) => {
    ...
    const status = await page.locator('[data-testid="call-status"]').textContent();
  ...
  test('should show call duration timer during active call', async ({ page }) => {
    // Initiate call
    await page.fill('[data-testid="phone-input"]', TEST_CONFIG.testPhoneNumber);
    await page.click('[data-testid="call-button"]');
  ...
  test('should display called number during call', async ({ page }) => {
    ...
    await expect(page.locator(`text=${TEST_CONFIG.testPhoneNumber}`)).toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe('Incoming Twilio Calls', () => {
  ...
  test('should show caller ID for incoming calls', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="caller-id"]')).toContainText('+19876543210', {
      timeout: 5000,
    });
  });
  ...
  test('should accept incoming call', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="call-status"]')).toContainText(/connected|active/i, {
      timeout: 5000,
    });
  });
...
test.describe('Call Status Updates', () => {
  ...
  test('should show connecting status when call is initiated', async ({ page }) => {
    // Initiate call
    await page.fill('[data-testid="phone-input"]', TEST_CONFIG.testPhoneNumber);
    await page.click('[data-testid="call-button"]');

    // Check for connecting status
    await expect(page.locator('[data-testid="call-status"]')).toContainText(/connecting/i, {
      timeout: 3000,
    });
  });
  ...
  test('should show ringing status when call is ringing', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="call-status"]')).toContainText(/ringing/i, {
      timeout: 3000,
    });
  });

  test('should show connected status when call is answered', async ({ page }) => {
    // Initiate call
    await page.fill('[data-testid="phone-input"]', TEST_CONFIG.testPhoneNumber);
    await page.click('[data-testid="call-button"]');
    ...
    await expect(page.locator('[data-testid="call-status"]')).toContainText(/connected/i, {
      timeout: 3000,
    });
  });
  ...
  test('should show ended status when call is completed', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="call-status"]')).toContainText(/ended|completed/i, {
      timeout: 5000,
    });
  });
  ...
  test('should show error status when call fails', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="call-status"]')).toContainText(/error|failed/i, {
      timeout: 3000,
    });
  });
});
