...
describe("User Model", () => {
  ...
  describe("Indexes", () => {
    test("should have firebaseUid unique index", () => {
      const indexes = [
        { fields: { firebaseUid: 1 }, options: { unique: true } },
      ];
...
describe("Recording Model", () => {
  ...
  describe("Instance Methods", () => {
    test("should have hasAccess method", () => {
      ...
      const checkAccess = (recording, firebaseUid) => {
        ...
        if (recording.accessControl.allowedUsers.includes(firebaseUid))
          return true;
        if (
          recording.accessControl.expiresAt &&
          new Date() > recording.accessControl.expiresAt
        ) {
          return false;
        }
        return false;
    ...
    test("should have getAccessUrl method", () => {
      const getAccessUrl = (recording) => {
        ...
        if (
          recording.storage.provider === "twilio" &&
          recording.storage.twilioUrl
        ) {
  ...
  describe("Pre-save Hooks", () => {
    test("should set listenedAt when isListened becomes true", () => {
      ...
      if (
        recording.isModified("isListened") &&
        recording.isListened &&
        !recording.listenedAt
      ) {
        recording.listenedAt = new Date();
      }
