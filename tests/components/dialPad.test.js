...
describe('DialPad Logic', () => {
  ...
  describe('Call button state', () => {
    it('should validate call button enable state', () => {
      const canMakeCall = (phoneNumber, disabled) => !disabled && phoneNumber.length > 0;
    ...
    it('should handle maximum phone number length', () => {
      const MAX_LENGTH = 15;
      const truncateNumber = (number) => number.slice(0, MAX_LENGTH);
...
describe('DialPad Component Logic', () => {
  describe('Input field state', () => {
    ...
    it('should handle undefined phoneNumber with fallback to empty string', () => {
      ...
      const activePhoneNumberBefore = setPhoneNumber ? phoneNumber : localPhoneNumber;

      // After fix: always returns empty string fallback
      const activePhoneNumberAfter = (setPhoneNumber ? phoneNumber : localPhoneNumber) || '';
