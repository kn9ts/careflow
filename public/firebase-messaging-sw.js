  ...
  try {
    ...
    messaging.onBackgroundMessage((payload) => {
      ...
      const notificationOptions = {
        ...
        tag: payload.data?.callSid || payload.data?.recordingSid || 'careflow-notification',
        actions:
...
self.addEventListener('notificationclick', (event) => {
  ...
  if (action === 'answer') {
    ...
    event.waitUntil(
      clients.openWindow('/dashboard?action=answer&callSid=' + notificationData.callSid)
    );
  } else if (action === 'decline') {
    // Open dashboard and trigger decline action
    event.waitUntil(
      clients.openWindow('/dashboard?action=decline&callSid=' + notificationData.callSid)
    );
  } else {
    ...
    if (notificationData.type === 'missed_call' || notificationData.type === 'voicemail') {
      url += '?tab=history';
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const notificationOptions = {
      ...
      data: data.data || {},
      requireInteraction: true,
      tag: data.data?.callSid || data.data?.recordingSid || 'careflow-notification',
      actions:
