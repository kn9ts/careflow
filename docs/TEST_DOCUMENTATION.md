...
├── webrtc.test.js              # WebRTC Manager unit tests
├── webrtc-integration.test.js  # WebRTC signaling integration tests
...
│   └── webrtc.test.js         # WebRTC configuration, signaling, quality metrics
...
```javascript
import { validateRegistrationInput, getAuthErrorMessage, AUTH_ERROR_CODES } from '@/lib/auth';
...
### WebRTC Mock Configuration

WebRTC tests require additional browser API mocks that must be set up before module import:

```javascript
// In beforeAll() - order matters!
beforeAll(async () => {
  // 1. Set up global window object
  global.window = {};

  // 2. Mock RTCPeerConnection
  ...
  // 5. Mock navigator.mediaDevices
  global.navigator = {
    mediaDevices: {
      getUserMedia: jest.fn().mockResolvedValue({
        getTracks: () => [{ kind: 'audio', enabled: true, stop: jest.fn() }],
        getAudioTracks: () => [{ kind: 'audio', enabled: true }],
      }),
    },
  };

  // 6. Mock MediaRecorder
  global.MediaRecorder = class MediaRecorder {
    static isTypeSupported = (mimeType) => ['audio/webm'].includes(mimeType);
    start() {
  ...
  // 7. Import module AFTER all mocks are set up
  const module = await import('@/lib/webrtc.js');
  WebRTCManager = module.default;
});
```

See `docs/WEBRTC_DOCUMENTATION.md` for complete WebRTC testing documentation.
...
- **WebRTC Testing Infrastructure**: Added comprehensive mocks for browser WebRTC APIs
  - `navigator.mediaDevices.getUserMedia` mock for microphone access
  - `RTCPeerConnection` mock with full ICE/SDP handling
  - `RTCSessionDescription` and `RTCIceCandidate` mocks
  - `MediaRecorder` mock for call recording tests
- **Test File Updates**:
  - `tests/webrtc.test.js`: Updated to use `global.window` for `isWebRTCSupported()` check
  - `tests/webrtc-integration.test.js`: Fixed mock setup order and navigator mock
- **Documentation**: Added WebRTC testing section to `docs/WEBRTC_DOCUMENTATION.md`
- **Error Message Updates**: Changed "WebRTC is only available in browser environment" to "WebRTC is not supported in this browser" for consistency
