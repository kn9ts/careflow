...
class CallManager {
  ...
  async startRecording() {
    ...
    try {
      ...
      logger.error('CallManager', `Failed to start recording: ${error.message}`);
      if (this.listeners.onRecordingError) {
  ...
  async stopRecording() {
    ...
    try {
      ...
      logger.error('CallManager', `Failed to stop recording: ${error.message}`);
      if (this.listeners.onRecordingError) {
        this.listeners.onRecordingError({
  ...
  async uploadRecording(recordingBlob, metadata = {}) {
    ...
    try {
      ...
      logger.error('CallManager', `Failed to upload recording: ${error.message}`);
      throw error;
    }
  }
