...
class CallManager {
  ...
  async makeCall(number) {
    ...
    // Determine which mode to use for this call (async for phone lookup)
    const callMode = await this._determineCallMode(destination);
    ...
    try {
      ...
      // Use resolved care4wId if available (from phone lookup)
      const webRTCDestination = this._resolvedCare4wId || destination;
      this._resolvedCare4wId = null; // Clear after use
      return await this.makeWebRTCCall(webRTCDestination);
    } catch (error) {
      this._updateConnectionState(
        'failed',
  ...
  async _determineCallMode(destination) {
    ...
    if (isCareFlowId) {
      ...
      if (snapshot.twilioReady && isPhoneNumber) {
        logger.debug('CallManager', 'Using Twilio for phone number call');
        return 'twilio';
      }

      // If Twilio is not available but destination is a phone number,
      // try to look up the user by phone number for WebRTC call
      if (isPhoneNumber && snapshot.webrtcReady) {
        logger.debug('CallManager', 'Looking up user by phone number for WebRTC call');
        const lookupResult = await this._lookupUserByPhone(destination);
        if (lookupResult.exists && lookupResult.care4wId) {
          // Store the resolved care4wId for use in makeWebRTCCall
          this._resolvedCare4wId = lookupResult.care4wId;
          logger.debug(
            'CallManager',
      ...
      // Cannot call phone numbers without Twilio or phone lookup
      if (isPhoneNumber) {
        throw new Error(
   ...
   * @returns {Promise<{exists: boolean, care4wId?: string, displayName?: string}>}
   * @private
   */
  async _lookupUserByPhone(phoneNumber) {
    try {
      const response = await fetch(
        `/api/users/lookup/phone?phoneNumber=${encodeURIComponent(phoneNumber)}`,
        {
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
      ...
      if (!response.ok) {
        logger.error('CallManager', 'Phone lookup failed');
        return { exists: false };
      }
      ...
      logger.error('CallManager', `Phone lookup error: ${error.message}`);
      return { exists: false };
    }
  }
