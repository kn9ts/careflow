...
class CallManager {
  ...
  async initialize(token, care4wId) {
    ...
    // RC-100: Create promise immediately to close timing window
    ...
    try {
      ...
      // RC-103: Keep promise cached for integrity, don't clear on success
      ...
      this._initialized = false;
      // RC-103: Clear promise only on failure to allow retry
      this._initializationPromise = null;
      this._initializationError = error;
  ...
  async _doInitialize(token, care4wId) {
    // RC-101: Lock mode during initialization
    if (this._modeLocked) {
    ...
    try {
      ...
      let tokenInfo;
      try {
        // RC-102: Use deduplicated token fetch
      ...
      // RC-101: Release mode lock after initialization
      this._modeLocked = false;
    }
  }
  ...
  async initializeTwilio(twilioToken) {
    ...
    try {
      ...
      logger.error('TwilioDevice', `Initialization failed: ${error?.message || error || 'Unknown error'}`);
      this._updateConnectionState('failed', `Twilio init failed: ${error?.message || error || 'Unknown error'}`);
  ...
  _setupTwilioDeviceHandlers() {
    ...
    this.twilioDevice.on('incoming', (conn) => {
      ...
      conn.on('error', (error) => {
        ...
        this._updateConnectionState('failed', `Call error: ${error?.message || error || 'Unknown error'}`);
        this.notifyStatusChange('idle');
    ...
    this.twilioDevice.on('error', (error) => {
      ...
      if (error.code === 20101) {
        ...
        this._updateConnectionState('failed', `Device error: ${error?.message || error || 'Unknown error'}`);
      }
  ...
  async initializeWebRTC() {
    ...
    try {
      ...
      logger.error('WebRTCManager', `Initialization failed: ${error?.message || error || 'Unknown error'}`);
      this._updateConnectionState('failed', `WebRTC init failed: ${error?.message || error || 'Unknown error'}`);
  ...
  async makeCall(number) {
    ...
    try {
      ...
      this._updateConnectionState('failed', `Call failed: ${error?.message || error || 'Unknown error'}`);
      throw error;
    }
  }
  ...
  async acceptWebRTCCall(roomId, offer) {
    ...
    try {
      ...
      this._updateConnectionState('failed', `Failed to accept: ${error?.message || error || 'Unknown error'}`);
      throw error;
    }
  }
  ...
  async startRecording() {
    ...
    try {
      ...
      logger.error('CallManager', `Failed to start recording: ${error?.message || error || 'Unknown error'}`);
      if (this.listeners.onRecordingError) {
  ...
  async stopRecording() {
    ...
    try {
      ...
      logger.error('CallManager', `Failed to stop recording: ${error?.message || error || 'Unknown error'}`);
      if (this.listeners.onRecordingError) {
        this.listeners.onRecordingError({
  ...
  async uploadRecording(recordingBlob, metadata = {}) {
    ...
    try {
      ...
      logger.error('CallManager', `Failed to upload recording: ${error?.message || error || 'Unknown error'}`);
      throw error;
    }
  }
