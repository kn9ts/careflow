...
async function getNextSequence(providedConnection = null) {
  ...
  if (!connection) {
    try {
      ...
      console.warn('Could not access mongoose connection:', error?.message || error || 'Unknown error');
    }
  }
  ...
  if (connection) {
    try {
      ...
      console.warn('Counter collection not available, using fallback:', error?.message || error || 'Unknown error');
    }
  }
