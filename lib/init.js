...
class ApplicationInitializer {
  ...
  async _performInitialization() {
    try {
      ...
      if (process.env.NODE_ENV === "production") {
        console.error(
          "💥 Production environment: Configuration errors are fatal",
        );
        process.exit(1);
      }
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.log(
          "- Refer to .env.local.example for required variable names",
        );
  ...
  _logInitializationStatus(readiness) {
    ...
    if (readiness.allReady) {
      ...
      console.log(
        "⚠️  Some services are missing - application may have limited functionality",
      );
    }
  }
  ...
  _setupGlobalState(readiness) {
    ...
    global.APP_CONFIG = {
      get: (key) => {
        if (!this.isInitialized) {
          throw new Error(
            "Application not initialized. Call appConfig.initialize() first.",
          );
        }
  ...
  async _setupEnvironmentSpecificFeatures() {
    ...
    switch (environment) {
      ...
      default:
        console.warn(
          `⚠️  Unknown environment: ${environment}, using development defaults`,
        );
        await this._setupDevelopmentFeatures();
    }
  }
...
export function getAppConfig() {
  if (!initializer.isInitialized) {
    throw new Error(
      "Application not initialized. Call initializer.initialize() first.",
    );
  }
  return require("./env.config.js").default;
}
