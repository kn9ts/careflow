...
export class AudioProcessor {
  ...
  async initialize() {
    try {
      ...
      this.localStream = await navigator.mediaDevices.getUserMedia(OPTIMIZED_AUDIO_CONSTRAINTS);
  ...
  createPeer(initiator = true, signalData = null) {
    ...
    this.peer.on('data', (data) => {
      logger.trace('AudioProcessor', `Data received: ${JSON.stringify(data)}`);
    });
  ...
  async processRecording() {
    ...
    logger.success('AudioProcessor', `Recording processed: ${recording.size} bytes`);
    return recording;
  }
  ...
  async ffmpegConvert(webmBlob) {
    ...
    await ffmpeg.exec(['-i', inputName, '-c:a', 'libopus', '-b:a', '128k', outputName]);
...
export class RecordingUploader {
  ...
  async upload(recording) {
    ...
    while (attempts < this.maxRetries) {
      try {
        logger.loading('RecordingUploader', `Upload attempt ${attempts + 1}...`);
    ...
    logger.error('RecordingUploader', `Upload failed after ${this.maxRetries} attempts`);
