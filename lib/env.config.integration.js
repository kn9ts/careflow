...
import config, { configManager } from './env.config.js';
...
class ApplicationConfig {
  ...
  async initialize() {
    ...
    try {
      ...
      if (config._meta.warnings.length > 0) {
        ...
        config._meta.warnings.forEach((warning) => console.warn(`  - ${warning}`));
      }
  ...
  checkServiceReadiness() {
    const services = {
      ...
      backblaze: !!config.BACKBLAZE_KEY_ID,
    };

    const readyServices = Object.entries(services).filter(([_, ready]) => ready);
    const missingServices = Object.entries(services).filter(([_, ready]) => !ready);
...
export function createErrorMiddleware() {
  return (error, req, res, next) => {
    ...
    // If headers haven't been sent, send response
    if (!res.headersSent) {
...
export async function startApplication() {
  try {
    ...
    console.log(`📊 Services ready: ${readiness.readyCount}/${readiness.totalServices}`);
