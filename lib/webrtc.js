...
const initializeFirebase = async () => {
  ...
  firebaseInitPromise = (async () => {
    try {
      ...
      logger.error('Firebase', `Initialization failed: ${error?.message || error || 'Unknown error'}`);
      throw error;
    }
  })();
...
class WebRTCManager {
    ...
    return withTimeout(
      new Promise((resolve, reject) => {
        try {
          ...
          logger.error('WebRTCManager', `Failed to create peer connection: ${error?.message || error || 'Unknown error'}`);
          reject(error);
        }
      }),
  ...
  async getLocalStream(constraints = { audio: true, video: false }) {
    try {
      ...
      this.localStream.getTracks().forEach((track) => {
        ...
        logger.debug('WebRTCManager', `Tracking ${track.kind} track: ${track.id}`);
      });
      ...
      logger.error('WebRTCManager', `Failed to get local stream: ${error?.message || error || 'Unknown error'}`);
  ...
  async sendIceCandidate(candidate) {
    ...
    try {
      ...
      logger.error('WebRTCManager', `Failed to send ICE candidate: ${error?.message || error || 'Unknown error'}`);
    }
  }
    ...
    const unsubscribe = onValue(answerRef, async (snapshot) => {
      ...
      if (answer && answer.sdp) {
        ...
        try {
          ...
          logger.error('WebRTCManager', `Failed to set remote description: ${error?.message || error || 'Unknown error'}`);
          this._updateState('failed', error?.message || error || 'Unknown error');
        }
      }
    });
      ...
      for (const [, candidateData] of Object.entries(candidates)) {
        if (candidateData.candidate && candidateData.from !== this.localCare4wId) {
          try {
            ...
            if (!error?.message || error || 'Unknown error'.includes('remote')) {
              logger.error('WebRTCManager', `Failed to add ICE candidate: ${error?.message || error || 'Unknown error'}`);
            }
          }
        }
      }
  ...
  async endCall() {
    ...
    if (this.db && this.currentRoomId && this.firebaseFns) {
      try {
        ...
        logger.warn('WebRTCManager', `Failed to cleanup room: ${error?.message || error || 'Unknown error'}`);
      }
      this.currentRoomId = null;
    }
  ...
  async attemptReconnection() {
    ...
    if (this.currentRoomId) {
      try {
        ...
        logger.error('WebRTCManager', `Reconnection attempt failed: ${error?.message || error || 'Unknown error'}`);
  ...
  async restartIce() {
    ...
    try {
      ...
      logger.error('WebRTCManager', `ICE restart failed: ${error?.message || error || 'Unknown error'}`);
      throw error;
    }
  }
  ...
  async getConnectionStats() {
    ...
    try {
      ...
      logger.error('WebRTCManager', `Failed to get stats: ${error?.message || error || 'Unknown error'}`);
      return null;
    }
  }
  ...
  async startRecording() {
    ...
    try {
      ...
      logger.error('WebRTCManager', `Recording failed: ${error?.message || error || 'Unknown error'}`);
