  ...
  if (!firebaseAuthInstance) {
    ...
    if (app) {
      try {
        ...
        // Use session persistence for better auth state persistence across page navigation
        // This ensures the user stays logged in during a session but not across browser restarts
        firebaseAuthInstance.setPersistence(browserSessionPersistence).catch((err) => {
          console.warn(
            'Failed to set session persistence, falling back to in-memory:',
            err.message
          );
          // Fall back to in-memory if session fails
