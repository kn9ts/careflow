...
export const logger = {
  ...
  ready: (service) =>
    log(service, "✨ Ready to rock!", "success", EMOJIS.party),
  ...
  callStart: (service, number) =>
    log(service, `📞 Calling ${number}...`, "info", EMOJIS.phone),
  callConnect: (service) =>
    log(service, "🎉 Connected!", "success", EMOJIS.party),
  ...
  // Audio/Recording
  recordingStart: (service) =>
    log(service, "🎙️ Recording started", "info", EMOJIS.recording),
  recordingStop: (service) =>
    log(service, "🛑 Recording stopped", "success", EMOJIS.recording),

  // Auth
  authStart: (service) =>
    log(service, "🔐 Authenticating...", "pending", EMOJIS.lock),
  authComplete: (service) =>
    log(service, "🔓 Authenticated!", "success", EMOJIS.key),

  // Debug/Trace
  debug: (service, message) =>
    log(service, message, "debug", EMOJIS.construction),
