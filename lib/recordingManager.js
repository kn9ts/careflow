  ...
  async stopRecording() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.mediaRecorder.onerror = (event) => {
        ...
        reject(new Error(`Recording stop failed: ${errorMessage}`));
      };
...
class RecordingUploader {
  ...
  async upload(recordingBlob, metadata, token) {
    const { callId, direction } = metadata;
    // Note: duration, from, to are available in metadata for future use
  ...
  async uploadToAPI(file, metadata, token) {
    ...
    if (!response.ok) {
      ...
      throw new Error(error?.message || error || 'Unknown error' || 'Upload failed');
    }
