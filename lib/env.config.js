 ...
 *
 * FIXED: Added client-side guard to prevent dotenv.config() from running in browser.
 */

// Import required modules (server-side only)
...
class EnvironmentValidator {
  ...
  validateVariable(key, value, schema) {
    ...
    if (schema.required && (!value || value.trim() === '')) {
      this.errors.push(`Required environment variable '${key}' is missing or empty`);
      return false;
    }
    ...
    if (typedValue === null) {
      this.errors.push(`Environment variable '${key}' must be of type ${schema.type}`);
      return false;
    }
    ...
    if (schema.enum && !schema.enum.includes(value)) {
      this.errors.push(`Environment variable '${key}' must be one of: ${schema.enum.join(', ')}`);
      return false;
    }
...
class ConfigurationManager {
   ...
   * NOTE: This only works on server-side (uses Node.js fs module)
   */
  loadEnvironment() {
    // CRITICAL: Skip on client-side
  ...
  buildConfiguration(env) {
    ...
    for (const [key, schema] of Object.entries(CONFIG_SCHEMA)) {
      ...
      if (!value && schema.default !== undefined) {
        ...
        this.validator.warnings.push(`Using default value for '${key}': ${schema.default}`);
      }
  ...
  initialize() {
    ...
    try {
      ...
      if (!validation.valid) {
        ...
        console.warn('Configuration validation failed but continuing (development mode)');
      }
      ...
      this.config = Object.freeze(configObj);
      this.isInitialized = true;

      // Log warnings
      if (this.validator.warnings.length > 0) {
        console.warn('Configuration warnings:');
        this.validator.warnings.forEach((warning) => console.warn(`  - ${warning}`));
      }

      console.log(`Configuration loaded successfully for ${configObj.NODE_ENV} environment`);
      return this.config;
    } catch (error) {
  ...
  new Proxy(
    {},
    {
      ...
      getOwnPropertyDescriptor(target, prop) {
        ...
        return Object.getOwnPropertyDescriptor(configObj, prop);
      },
    }
  );
...
// dotenv.config() uses Node.js fs module which is not available in browsers
if (typeof window === 'undefined') {
