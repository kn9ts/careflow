...
async function sendFCMMessage(token, notification, data = {}) {
  ...
  try {
    ...
    if (serviceAccountKey) {
      ...
      console.warn('Firebase service account key not configured, using fallback');
...
async function getAccessToken(serviceAccountKey) {
  ...
  throw new Error('Use legacy FCM API - proper JWT signing requires crypto library');
}
 ...
 * @returns {Promise<Object>} Result of notification sending
 */
export async function sendCallStatusNotification(firebaseUid, status, callData) {
